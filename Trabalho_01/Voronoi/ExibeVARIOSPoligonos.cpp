...
void display(void)
{
    ...
    for (int i = 0; i < Voro.getNPoligonos(); i++)
    {
        Poligono Map = Voro.getPoligono(i);
        for (int j = 0; j < Map.getNVertices(); j++)
        {
            Map.getAresta(j, P1, P2);
...
{
    Ponto pontoMenor, pontoMaior;
    p.obtemLimites(pontoMenor, pontoMaior);
    Envelope envelope = Envelope(pontoMenor, pontoMaior);
    return envelope.pontoEstaDentro(pt);
    //  if (pt.x > pontoMenor.x && pt.x < pontoMaior.x) {
    //      if (pt.y > pontoMenor.y && pt.y < pontoMaior.y) {
    //          return true;
    //      }
    //  }
    // return false;
